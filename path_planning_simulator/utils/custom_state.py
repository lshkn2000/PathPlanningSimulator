...
class FeaturedLSTM:
    def __init__(self, input_dim=5, output_dim=10+7, hidden_dim=30, num_layers=2):
        self.lstm = LSTM(input_dim=input_dim, output_dim=output_dim - 7, hidden_dim=hidden_dim, num_layers=num_layers)
    ...
    def custom_state_for_lstm(self, state):
        if state.ndim == 1: # numpy : 시뮬레이션에서 numpy로 받아
            ...
            obstacles_states = obstacles_states.reshape(batch_size, obstacles_num, 5)
            # robot_state_tensor = torch.tensor(robot_state, dtype=torch.float32)
            # obstacles_states = torch.tensor(obstacles_states, device=device, dtype=torch.float32)
