...
class PretrainedSim(object):
    ...
    def pretrain_with_vae_latent(self, latent_dataset_dir=None, pretrain_episodes=5000):
        # set transitions to replay buffer
        if latent_dataset_dir is not None:
            print("Loading Latent Dataset ...")
            LATENT_DATASET_PATH = latent_dataset_dir
            if os.path.isfile(LATENT_DATASET_PATH):
                with open(LATENT_DATASET_PATH, 'rb') as f:
                    latent_dict = pickle.load(f)
            else:
                raise Exception(f"Latent Dataset is not exist in that path : {LATENT_DATASET_PATH}")

            z = np.array(latent_dict['z']).squeeze()
            # z = torch.from_numpy(z).to(torch.float32).squeeze()     # [steps, z_size]

            buffer = itertools.islice(self.pretraied_replay_buffer, 0, len(self.pretraied_replay_buffer)-1)
            for idx, (state, action, reward, new_state, is_terminal) in tqdm(enumerate(buffer), desc="Transform State with VAE Latent Variable"):
                self.env.robot.policy.store_trajectory(z[idx], action, reward, z[idx + 1], is_terminal)
                # self.env.robot.policy.store_trajectory(np.hstack((z[idx], state)), action, reward, np.hstack((z[idx+1], new_state)), is_terminal)
        else:
